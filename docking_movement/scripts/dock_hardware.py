...
def xyt_callback(data:PoseStamped):
    global xyt
    x_offset=0.22
    y_offset=0
...
def turn(angle):
    ...
    while abs(t_est-angle)>math.radians(1): #set angle threshold
        ...
        t_est+=dtheta*dt
        cmd.angular.z=0.4*sign_function(angle) #edit sign later 
        pub.publish(cmd)
        rate.sleep()
    cmd.linear.x=0
    cmd.angular.z=0 
...
if __name__=='__main__':
    ...
    odosub=rospy.Subscriber('/raw_odom',Odometry,callback=odo_callback)
    ...
    set_distance=2
    while not (abs(xyt.y)<0.1 and abs(xyt.theta)<=math.radians(10)):
